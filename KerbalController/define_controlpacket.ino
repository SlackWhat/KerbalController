...
struct ControlPacket {
  byte id;
  byte MainControls;                  // SAS, RCS, Lights, Gears, Brakes, Precision, Abort, Stage...
...
void define_control_packet() {
  if (Connected) {
    ...
    navballMode = getNavballMode();
    if (!digitalRead(pTB)) {
      navballMode++;
      if (sasMode == 3)
        sasMode = 1;
      setNavballMode(sasMode);
    }
...
/*
    ...
    if (rb_on) {
    */
      //rocket mode   MODIFIED !!!!!
      //if (analogRead(pRX) >= 530) { cPacket.Yaw = constrain(map(analogRead(pRX),1023,530,0,1000),0,1000); }
      //else if (analogRead(pRX) <= 470) { cPacket.Yaw = constrain(map(analogRead(pRX),470,0,-1000,0),-1000,0); }
      //else { cPacket.Yaw = 0; }
      ...
      if (analogRead(pRZ) <= 470) { CPacket.Roll = constrain(map(analogRead(pRZ),0,470,-1000,0),-1000,0); }
      else if (analogRead(pRZ) >= 530) { CPacket.Roll = constrain(map(analogRead(pRZ),530,1023,0,1000),0,1000); }
      else { CPacket.Roll = 0; }

      if (analogRead(pTX) >= 530) { CPacket.TX = constrain(map(analogRead(pTX),1023,530,1000,0),0,1000); }
      else if (analogRead(pTX) <= 470) { CPacket.TX = constrain(map(analogRead(pTX),470,0,0,-1000),-1000,0); }
      else { CPacket.TX = 0; }
      ...
      if (analogRead(pTZ) <= 470) { CPacket.TZ = constrain(map(analogRead(pTZ),0,470,-1000,0),-1000,0); }
      else if (analogRead(pTZ) >= 530) { CPacket.TZ = constrain(map(analogRead(pTZ),530,1023,0,1000),0,1000); }
      else { CPacket.TZ = 0; }
    /*}
    else {
      //airplane mode
